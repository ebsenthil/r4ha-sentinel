"""
R4HA Data Generator - 1 Hour Prediction Horizon
Generates synthetic mainframe MSU data with all 15 TIER 1 features
Target: Predict R4HA 1 hour ahead

File: r4ha_data_generator_1h.py
"""
...
class R4HADataGenerator:
    ...
    def __init__(self, system_id='PROD01', start_date='2024-01-01', months=12, seed=42):
        ...
        self.batch_windows = [
            {'name': 'nightly_batch', 'start': 0, 'end': 6, 'days': [0,1,2,3,4,5,6]},
            {'name': 'eod_processing', 'start': 18, 'end': 22, 'days': [0,1,2,3,4]},
            {'name': 'weekend_extended', 'start': 22, 'end': 10, 'days': [5,6]}
        ]
    ...
    def _calculate_base_msu(self, timestamp):
        ...
        # Hourly pattern
        hourly_pattern = {
            0: 0.9, 1: 0.95, 2: 1.0, 3: 1.0, 4: 0.95, 5: 0.9,
            6: 0.7, 7: 0.6, 8: 0.75, 9: 0.85, 10: 0.9,
            11: 0.95, 12: 0.9, 13: 0.92, 14: 0.95, 15: 0.93,
            16: 0.9, 17: 0.85, 18: 0.95, 19: 1.0, 20: 0.98, 21: 0.95,
            22: 0.85, 23: 0.88
        }
        
        # Weekly pattern
        weekly_pattern = {
            0: 1.1, 1: 1.0, 2: 1.0, 3: 1.0, 4: 1.05, 5: 0.95, 6: 0.7
        }
    ...
    def generate_raw_data(self):
        ...
        for i, ts in enumerate(timestamps):
            ...
            cpu_utilization_pct += np.random.normal(0, 3)
    ...
    def calculate_features(self, df):
        """Calculate all derived features for 1-hour prediction"""
        print("\nCalculating derived features (1-hour prediction horizon)...")
        
        df = df.sort_values('timestamp').reset_index(drop=True)
        
        # Calculate R4HA (4-hour rolling average)
        ...
        df['r4ha_lag_1h'] = df['r4ha_msu'].shift(4)
        df['r4ha_lag_2h'] = df['r4ha_msu'].shift(8)
        ...
        # Create target variable (R4HA 1 hour ahead)
        print("  - Creating target variable (R4HA 1 hour ahead = 4 periods)...")
        df['target'] = df['r4ha_msu'].shift(-4)  # 1 hour = 4 periods of 15 minutes
    ...
    def print_data_summary(self, df):
        ...
        print(f"Target (R4HA 1h ahead) - Mean: {df['target'].mean():.2f}")
        ...
        print(f"Avg Jobs (Batch): {df[df['is_batch_window']==1]['batch_jobs_running'].mean():.1f}")
...
def main():
    ...
    print("\n--- Sample: Current vs 1-hour Ahead Target ---")
    sample = df[['timestamp', 'r4ha_msu', 'target']].head(20)
    sample.columns = ['Time', 'Current R4HA', 'R4HA in 1h']
...
if __name__ == "__main__":
    ...
    print("\n→ Next step: Run r4ha_train_model_1h.py")